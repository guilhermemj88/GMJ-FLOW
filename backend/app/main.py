...
BGP_ACTIONS = {"alert_only", "discard", "accept", "rate_limit", "redirect", "redirect_to_vrf", "set_community", "community", "blackhole", "announce_blackhole", "announce_route"}
BGP_FLOWSPEC_ACTIONS = {"discard", "accept", "rate_limit"}
BGP_TARGET_SELECTORS = {"src_ip", "dst_ip", "src_and_dst_ip", "dst_prefix", "src_prefix", "conversation", "target_ip", "target_cidr", "anomaly_src_ip", "anomaly_dst_ip"}
...
class DetectionRulePayload(BaseModel):
    ...
    output_if: str = ""
    sensor_id: int | None = Field(None, ge=1)
    warning_response_profile_id: int | None = Field(None, ge=1)
    critical_response_profile_id: int | None = Field(None, ge=1)
    fallback_response_profile_id: int | None = Field(None, ge=1)
    mitigation_mode: str = "detection_only"
    mitigation_enabled: bool = False
    max_auto_prefixlen_v4: int = Field(32, ge=0, le=32)
    max_auto_prefixlen_v6: int = Field(128, ge=0, le=128)
    require_protected_prefix: bool = True
    cooldown_seconds: int | None = Field(None, ge=0, le=86400)
    duration_seconds: int = Field(7200, ge=60, le=604800)
    max_active_announcements: int = Field(10, ge=1, le=10000)
    min_confidence_for_auto: float = Field(0.90, ge=0, le=1)
    use_global_whitelist: bool = True
    extra_whitelist_ids: list[int] = Field(default_factory=list)
    bypass_whitelist: bool = False
    notes: str = ""
...
class BgpConnectorPayload(BaseModel):
    ...
    local_address: str = ""
    router_id: str = ""
    passive_listen_enabled: bool = False
    listen_port: int = Field(179, ge=1, le=65535)
    default_next_hop: str = ""
    default_community: str = ""
    default_large_community: str = ""
    exabgp_config_path: str = ""
    exabgp_pipe_in: str = ""
    exabgp_pipe_out: str = ""
    systemd_service_name: str = ""
    router_check_enabled: bool = False
    router_vendor: str = "huawei_vrp"
    router_mgmt_ip: str = ""
    router_username: str = ""
    router_auth_secret_ref: str = ""
    router_password: str = ""
...
class BgpResponseProfilePayload(BaseModel):
    ...
    rate_limit_value_raw: str | None = None
    rate_limit_unit: str | None = None
    default_rate_limit_bps: int | None = Field(None, ge=1)
...
class BgpFlowspecTestPayload(BaseModel):
    action: str | None = "dry_run"
    src_cidr: str | None = None
    dst_cidr: str | None = None
    protocol: str | None = "any"
    src_port: Any = None
    dst_port: Any = None
    then_action: str | None = "discard"
    rate_limit_value_raw: str | None = None
    rate_limit_bps: int | None = Field(None, ge=1)
    duration_seconds: int | None = Field(None, ge=60, le=604800)
    confirm: str = ""
...
class BgpMitigationPortPolicyPayload(BaseModel):
    protocol: str = "udp"
    port_kind: str = "source"
    port_value: int = Field(..., ge=1, le=65535)
    name: str
    is_critical: bool = True
    allow_port_only_auto: bool = False
    require_ip_scope: bool = True
    allow_with_dst_protected_prefix: bool = True
    allow_with_src_ip: bool = True
    allow_with_dst_ip: bool = True
    min_dst_prefixlen_auto: int | None = Field(None, ge=0, le=128)
    min_src_prefixlen_auto: int | None = Field(None, ge=0, le=128)
    description: str = ""
    is_active: bool = True
...
def ensure_attack_vector_db(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS anomaly_events (
            ...
            invalid_scope INTEGER NOT NULL DEFAULT 0,
    ...
    ensure_sqlite_column(conn, "anomaly_events", "zone_id", "zone_id INTEGER")
    ensure_sqlite_column(conn, "anomaly_events", "zone_name", "zone_name TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "anomaly_events", "vector_name", "vector_name TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "anomaly_events", "scope_type", "scope_type TEXT NOT NULL DEFAULT ''")
    ...
    ensure_sqlite_column(conn, "anomaly_events", "detection_engine", "detection_engine TEXT NOT NULL DEFAULT 'legacy'")
    ensure_sqlite_column(conn, "anomaly_events", "detection_template_id", "detection_template_id INTEGER")
    ensure_sqlite_column(conn, "anomaly_events", "detection_template_rule_id", "detection_template_rule_id INTEGER")
...
def ensure_ip_zone_detection_db(conn: sqlite3.Connection) -> None:
    ...
    ensure_sqlite_column(conn, "security_anomalies", "target_ip", "target_ip TEXT")
    ensure_sqlite_column(conn, "security_anomalies", "target_cidr", "target_cidr TEXT")
    ...
    detection_rule_columns = {
        ...
        "dst_port": "dst_port TEXT NOT NULL DEFAULT 'any'",
        "tcp_flags": "tcp_flags TEXT NOT NULL DEFAULT ''",
        "icmp_type": "icmp_type TEXT NOT NULL DEFAULT ''",
        "icmp_code": "icmp_code TEXT NOT NULL DEFAULT ''",
        "detection_key": "detection_key TEXT NOT NULL DEFAULT ''",
        "group_by": "group_by TEXT NOT NULL DEFAULT ''",
        "scope": "scope TEXT NOT NULL DEFAULT ''",
        "input_if": "input_if TEXT NOT NULL DEFAULT ''",
        "output_if": "output_if TEXT NOT NULL DEFAULT ''",
        "sensor_id": "sensor_id INTEGER",
        "warning_response_profile_id": "warning_response_profile_id INTEGER",
        "critical_response_profile_id": "critical_response_profile_id INTEGER",
        "fallback_response_profile_id": "fallback_response_profile_id INTEGER",
        "mitigation_mode": "mitigation_mode TEXT NOT NULL DEFAULT 'detection_only'",
        "mitigation_enabled": "mitigation_enabled INTEGER NOT NULL DEFAULT 0",
        "max_auto_prefixlen_v4": "max_auto_prefixlen_v4 INTEGER NOT NULL DEFAULT 32",
        "max_auto_prefixlen_v6": "max_auto_prefixlen_v6 INTEGER NOT NULL DEFAULT 128",
        "require_protected_prefix": "require_protected_prefix INTEGER NOT NULL DEFAULT 1",
        "cooldown_seconds": "cooldown_seconds INTEGER NOT NULL DEFAULT 300",
        "duration_seconds": "duration_seconds INTEGER NOT NULL DEFAULT 7200",
        "max_active_announcements": "max_active_announcements INTEGER NOT NULL DEFAULT 10",
        "min_confidence_for_auto": "min_confidence_for_auto REAL NOT NULL DEFAULT 0.90",
        "use_global_whitelist": "use_global_whitelist INTEGER NOT NULL DEFAULT 1",
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS detection_template_rule_responses (
            ...
            min_bits_s REAL,
            min_confidence REAL,
            required_evidence_status TEXT NOT NULL DEFAULT '',
            required_attack_shape TEXT NOT NULL DEFAULT '',
            response_profile_id INTEGER,
            priority INTEGER NOT NULL DEFAULT 100,
            enabled INTEGER NOT NULL DEFAULT 1,
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL,
            FOREIGN KEY(rule_id) REFERENCES detection_template_rules(id) ON DELETE CASCADE,
            FOREIGN KEY(response_profile_id) REFERENCES bgp_response_profiles(id) ON DELETE SET NULL
        )
        """
    )
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_detection_template_rule_responses_rule ON detection_template_rule_responses(rule_id, enabled, priority)")
...
def seed_default_bgp_response_profiles(conn: sqlite3.Connection) -> None:
    ...
    defaults = [
        ...
        {
            "name": "FLOWSPEC_BLOCK_SRC_DNS_RESPONSE",
            "description": "Bloqueia respostas DNS/UDP por source-port 53 com escopo de origem.",
            "response_type": "flowspec",
            "approval_mode": "manual_approval",
            "action": "discard",
            "target_selector": "src_ip",
            "protocol_selector": "udp",
            "src_port_selector": "fixed",
            "src_port_value": "53",
            "require_protocol_or_port": 1,
        },
        {
            "name": "FLOWSPEC_BLOCK_DST_DNS_QUERY",
            "description": "Bloqueia queries DNS/UDP por destination-port 53 com escopo de destino.",
            "response_type": "flowspec",
            "approval_mode": "manual_approval",
            "action": "discard",
        ...
        {
            "name": "FLOWSPEC_BLOCK_SRC_IP_ALL",
            "description": "Bloqueia todo trafego de uma origem especifica.",
            "response_type": "flowspec",
            "approval_mode": "manual_approval",
            "action": "discard",
            "target_selector": "src_ip",
        ...
        {
            "name": "FLOWSPEC_BLOCK_DST_IP_ALL",
            "description": "Bloqueia todo trafego para um destino especifico.",
            "response_type": "flowspec",
            "approval_mode": "manual_approval",
            "action": "discard",
            "target_selector": "dst_ip",
        ...
        {
            "name": "FLOWSPEC_BLOCK_DST_UDP_PORT",
            "description": "Bloqueia destino UDP /32 por destination-port a partir da anomalia.",
            "response_type": "flowspec",
            "approval_mode": "manual_approval",
            "action": "discard",
            "target_selector": "dst_ip",
    ...
    for item in defaults:
        ...
        conn.execute(
            ...
            (
                ...
                item.get("src_port_selector", "any"),
                item.get("src_port_value", ""),
...
def seed_default_bgp_port_policies(conn: sqlite3.Connection) -> None:
    now = utc_now_iso()
    defaults = [
        ("udp", "source", 53, "DNS_RESPONSE_REFLECTION", 1, 1, 32, 128, "DNS reflection/amplification por source-port 53."),
        ("udp", "destination", 53, "DNS_QUERY", 1, 1, 32, 128, "DNS query por destination-port 53."),
        ("udp", "source", 123, "NTP_REFLECTION", 1, 1, 32, 128, "NTP reflection por source-port 123."),
        ("udp", "destination", 123, "NTP", 1, 1, 32, 128, "NTP por destination-port 123."),
        ("tcp", "destination", 179, "BGP", 1, 1, 32, 128, "BGP TCP/179."),
        ("tcp", "destination", 22, "SSH", 1, 1, 32, 128, "SSH TCP/22."),
        ("udp", "destination", 161, "SNMP", 1, 1, 32, 128, "SNMP UDP/161."),
        ("tcp", "destination", 80, "HTTP", 1, 1, 32, 128, "HTTP TCP/80."),
        ("tcp", "destination", 443, "HTTPS", 1, 1, 32, 128, "HTTPS TCP/443."),
    ]
    for protocol, port_kind, port_value, name, critical, require_scope, min_dst, min_src, description in defaults:
        row = conn.execute(
            """
            SELECT id
            FROM bgp_mitigation_port_policies
            WHERE protocol = ? AND port_kind = ? AND port_value = ?
            LIMIT 1
            """,
            (protocol, port_kind, port_value),
        ).fetchone()
        if row is not None:
            continue
        conn.execute(
            """
            INSERT INTO bgp_mitigation_port_policies (
                protocol, port_kind, port_value, name, is_critical, allow_port_only_auto,
                require_ip_scope, allow_with_dst_protected_prefix, allow_with_src_ip, allow_with_dst_ip,
                min_dst_prefixlen_auto, min_src_prefixlen_auto, description, is_active, created_at, updated_at
            )
            VALUES (?, ?, ?, ?, ?, 0, ?, 1, 1, 1, ?, ?, ?, 1, ?, ?)
            """,
            (protocol, port_kind, port_value, name, critical, require_scope, min_dst, min_src, description, now, now),
        )
...
def ensure_bgp_db(conn: sqlite3.Connection) -> None:
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS bgp_connectors (
            ...
            passive_listen_enabled INTEGER NOT NULL DEFAULT 0,
            listen_port INTEGER NOT NULL DEFAULT 179,
            ...
            exabgp_config_path TEXT NOT NULL DEFAULT '',
            exabgp_pipe_in TEXT NOT NULL DEFAULT '',
            exabgp_pipe_out TEXT NOT NULL DEFAULT '',
            systemd_service_name TEXT NOT NULL DEFAULT '',
            router_check_enabled INTEGER NOT NULL DEFAULT 0,
            router_vendor TEXT NOT NULL DEFAULT 'huawei_vrp',
            router_mgmt_ip TEXT NOT NULL DEFAULT '',
    ...
    ensure_sqlite_column(conn, "bgp_connectors", "local_address", "local_address TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_connectors", "passive_listen_enabled", "passive_listen_enabled INTEGER NOT NULL DEFAULT 0")
    ensure_sqlite_column(conn, "bgp_connectors", "listen_port", "listen_port INTEGER NOT NULL DEFAULT 179")
    ensure_sqlite_column(conn, "bgp_connectors", "exabgp_config_path", "exabgp_config_path TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_connectors", "exabgp_pipe_in", "exabgp_pipe_in TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_connectors", "exabgp_pipe_out", "exabgp_pipe_out TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_connectors", "systemd_service_name", "systemd_service_name TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_connectors", "router_check_enabled", "router_check_enabled INTEGER NOT NULL DEFAULT 0")
    ensure_sqlite_column(conn, "bgp_connectors", "router_vendor", "router_vendor TEXT NOT NULL DEFAULT 'huawei_vrp'")
    ensure_sqlite_column(conn, "bgp_connectors", "router_mgmt_ip", "router_mgmt_ip TEXT NOT NULL DEFAULT ''")
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS bgp_response_profiles (
            ...
            rate_limit_value_raw TEXT NOT NULL DEFAULT '',
            rate_limit_unit TEXT NOT NULL DEFAULT '',
            default_rate_limit_bps INTEGER,
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS bgp_announcements (
            ...
            policy_severity TEXT NOT NULL DEFAULT '',
            policy_reasons TEXT NOT NULL DEFAULT '[]',
            policy_warnings TEXT NOT NULL DEFAULT '[]',
            policy_required_scope TEXT NOT NULL DEFAULT '[]',
            policy_matched_port_policies TEXT NOT NULL DEFAULT '[]',
            FOREIGN KEY(connector_id) REFERENCES bgp_connectors(id) ON DELETE SET NULL,
            FOREIGN KEY(response_profile_id) REFERENCES bgp_response_profiles(id) ON DELETE SET NULL
        )
        """
    ...
    ensure_sqlite_column(conn, "bgp_announcements", "policy_reasons", "policy_reasons TEXT NOT NULL DEFAULT '[]'")
    ...
    ensure_sqlite_column(conn, "bgp_response_profiles", "rate_limit_value_raw", "rate_limit_value_raw TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_response_profiles", "rate_limit_unit", "rate_limit_unit TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_response_profiles", "default_rate_limit_bps", "default_rate_limit_bps INTEGER")
    ...
    ensure_sqlite_column(conn, "bgp_response_profiles", "fixed_protocol", "fixed_protocol TEXT NOT NULL DEFAULT ''")
    ensure_sqlite_column(conn, "bgp_response_profiles", "tcp_flags_value", "tcp_flags_value TEXT NOT NULL DEFAULT ''")
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS bgp_mitigation_port_policies (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            protocol TEXT NOT NULL DEFAULT 'udp',
            port_kind TEXT NOT NULL DEFAULT 'source',
            port_value INTEGER NOT NULL,
            name TEXT NOT NULL,
            is_critical INTEGER NOT NULL DEFAULT 1,
            allow_port_only_auto INTEGER NOT NULL DEFAULT 0,
            require_ip_scope INTEGER NOT NULL DEFAULT 1,
            allow_with_dst_protected_prefix INTEGER NOT NULL DEFAULT 1,
            allow_with_src_ip INTEGER NOT NULL DEFAULT 1,
            allow_with_dst_ip INTEGER NOT NULL DEFAULT 1,
            min_dst_prefixlen_auto INTEGER,
            min_src_prefixlen_auto INTEGER,
            description TEXT NOT NULL DEFAULT '',
            is_active INTEGER NOT NULL DEFAULT 1,
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL
...
def detection_rule_row_to_dict(row: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "icmp_type": item.get("icmp_type") or "",
        "icmp_code": item.get("icmp_code") or "",
        ...
        "duration_seconds": int(item.get("duration_seconds") or 7200),
        "max_active_announcements": int(item.get("max_active_announcements") or 10),
        "min_confidence_for_auto": float(item.get("min_confidence_for_auto") or 0.90),
...
def normalize_detection_rule_payload(payload: DetectionRulePayload) -> dict[str, Any]:
    ...
    src_port = normalize_detection_port_text(data.get("src_port"), "src_port")
    dst_port = normalize_detection_port_text(data.get("dst_port"), "dst_port")
    cooldown_seconds = data.get("cooldown_seconds")
    if cooldown_seconds is None:
        cooldown_seconds = int(data.get("cooldown_minutes") or 0) * 60
    ...
    return {
        ...
        "cooldown_seconds": non_negative_int(cooldown_seconds, "cooldown_seconds"),
        "enabled": 1 if data.get("enabled") else 0,
        "response": response,
        "src_cidr": normalize_optional_cidr(data.get("src_cidr"), "src_cidr") or "",
        "dst_cidr": normalize_optional_cidr(data.get("dst_cidr"), "dst_cidr") or "",
        "src_port": src_port,
        "dst_port": dst_port,
        "tcp_flags": clean_text(data.get("tcp_flags")).lower(),
        "icmp_type": clean_text(data.get("icmp_type")),
        "icmp_code": clean_text(data.get("icmp_code")),
        "detection_key": group_by,
        "group_by": group_by,
        ...
        "duration_seconds": positive_int(data.get("duration_seconds") or 7200, "duration_seconds"),
        "max_active_announcements": positive_int(data.get("max_active_announcements") or 10, "max_active_announcements"),
        "min_confidence_for_auto": float(data.get("min_confidence_for_auto") or 0.90),
        "use_global_whitelist": 1 if data.get("use_global_whitelist") else 0,
...
def validate_detection_rule_profile_refs(conn: sqlite3.Connection, data: dict[str, Any]) -> None:
...
def fetch_detection_template(conn: sqlite3.Connection, template_id: int, include_rules: bool = False) -> dict[str, Any]:
    ...
    if include_rules:
        rows = conn.execute(
            ...
            LEFT JOIN bgp_response_profiles fp ON fp.id = r.fallback_response_profile_id
            WHERE r.template_id = ?
            ORDER BY r.enabled DESC, r.vector, r.id
            """,
            (template_id,),
        ).fetchall()
...
def bgp_connector_row_to_dict(row: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "local_address": item.get("local_address") or "",
        "router_id": item.get("router_id") or "",
        "passive_listen_enabled": sqlite_bool(item.get("passive_listen_enabled")),
        "listen_port": int(item.get("listen_port") or 179),
        ...
        "exabgp_config_path": item.get("exabgp_config_path") or "",
        "exabgp_pipe_in": item.get("exabgp_pipe_in") or "",
        "exabgp_pipe_out": item.get("exabgp_pipe_out") or "",
        "systemd_service_name": item.get("systemd_service_name") or "",
        "router_check_enabled": sqlite_bool(item.get("router_check_enabled")),
        "router_vendor": item.get("router_vendor") or "huawei_vrp",
        ...
        "enabled": sqlite_bool(item.get("enabled")),
        "is_active": sqlite_bool(item.get("is_active", item.get("enabled", 1))),
        "notes": item.get("notes") or "",
        "created_at": item["created_at"],
        "updated_at": item["updated_at"],
    }
...
def bgp_response_profile_row_to_dict(row: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "target": item.get("target_selector") or "src_ip",
        "protocol_selector": item.get("protocol_selector") or "anomaly_protocol",
        "protocol_mode": item.get("protocol_selector") or "anomaly_protocol",
        "fixed_protocol": item.get("fixed_protocol") or "",
        "src_port_selector": item.get("src_port_selector") or "any",
        "src_port_mode": item.get("src_port_selector") or "any",
        "src_port_value": item.get("src_port_value") or "",
        "dst_port_selector": item.get("dst_port_selector") or "any",
        "dst_port_mode": item.get("dst_port_selector") or "any",
        "dst_port_value": item.get("dst_port_value") or "",
        "tcp_flags_selector": item.get("tcp_flags_selector") or "any",
        "tcp_flags_mode": item.get("tcp_flags_selector") or "any",
        "tcp_flags_value": item.get("tcp_flags_value") or "",
        "rate_limit_bps": default_rate_limit_bps,
        "rate_limit_default": item.get("default_rate_limit_raw") or item.get("rate_limit_value_raw") or "",
        "rate_limit_value_raw": item.get("rate_limit_value_raw") or item.get("default_rate_limit_raw") or "",
        "rate_limit_unit": item.get("rate_limit_unit") or rate_limit_unit_for_bps(default_rate_limit_bps),
        "default_rate_limit_bps": default_rate_limit_bps,
        ...
        "max_prefixlen_v4": int(item.get("max_prefixlen_v4") or 32),
...
def response_profile_operational_description(profile: dict[str, Any]) -> str:
    ...
    response_type = clean_text(profile.get("response_type") or profile.get("type"))
    if response_type == "flowspec" and target == "dst_ip":
        return "Bloqueia o destino dominante da anomalia usando FlowSpec." if action == "discard" else "Aplica FlowSpec ao destino dominante da anomalia."
    if response_type == "flowspec" and target == "src_ip":
        return "Aplica FlowSpec a origem da anomalia; em outbound pode ser alto risco se a origem for cliente interno."
    if response_type in {"diversion"}:
        return "Desvia o trafego do alvo selecionado para scrubbing/diversion."
    if response_type in {"rtbh", "blackhole"}:
        return "Anuncia blackhole para o alvo selecionado."
    if response_type in {"detection_only", "alert_only", "webhook"}:
        return "Nao anuncia BGP; gera alerta ou integracao externa."
    return "Gera candidato de resposta conforme seletores do profile."


def response_profile_placeholder_preview(profile: dict[str, Any]) -> str:
    response_type = clean_text(profile.get("response_type") or profile.get("type")).lower()
    if response_type in {"detection_only", "alert_only", "webhook"}:
        return "alert_only"
    target = clean_text(profile.get("target_selector") or profile.get("target") or "dst_ip")
    action = clean_text(profile.get("action") or profile.get("default_action") or "discard")
    protocol = clean_text(profile.get("fixed_protocol")) or (
        clean_text(profile.get("protocol_selector")) if clean_text(profile.get("protocol_selector")) in {"tcp", "udp", "icmp"} else "<anomaly.protocol>"
    )
    dst_port_selector = clean_text(profile.get("dst_port_selector") or "any")
    destination = "<anomaly.dominant_dst_ip>/32" if target in {"dst_ip", "src_and_dst_ip", "conversation"} else ""
    source = "<anomaly.src_ip>/32" if target in {"src_ip", "src_and_dst_ip", "conversation"} else ""
    match_parts = []
    if source:
        match_parts.append(f"source {source};")
    if destination:
        match_parts.append(f"destination {destination};")
    if protocol and protocol != "any":
        match_parts.append(f"protocol {protocol};")
    if dst_port_selector == "anomaly_dst_port":
        match_parts.append("destination-port =<anomaly.dominant_dst_port>;")
    elif dst_port_selector == "fixed" and clean_text(profile.get("dst_port_value")):
        match_parts.append(f"destination-port ={clean_text(profile.get('dst_port_value'))};")
    if action == "rate_limit":
        then = f"rate-limit {clean_text(profile.get('default_rate_limit_bps') or profile.get('rate_limit_bps') or '<rate_limit>')};"
    elif action in {"redirect", "redirect_to_vrf"}:
        then = f"redirect {clean_text(profile.get('community') or profile.get('bgp_community') or '<community>')};"
    else:
        then = "discard;" if action in {"discard", "blackhole", "announce_blackhole"} else f"{action};"
    if response_type == "diversion":
        return f"announce route <target_prefix> next-hop {clean_text(profile.get('next_hop') or profile.get('redirect_target') or '<next-hop>')}"
    return f"announce flow route {{ match {{ {' '.join(match_parts)} }} then {{ {then} }} }}"
...
def bgp_announcement_row_to_dict(row: sqlite3.Row | dict[str, Any], include_events: bool = False, conn: sqlite3.Connection | None = None) -> dict[str, Any]:
    ...
    if not action and "discard" in clean_text(item.get("rendered_command")).lower():
        action = "discard"
    if (item.get("response_type") or "") == "flowspec" or "flow route" in clean_text(item.get("rendered_command")).lower():
        action = normalize_flowspec_action(action or "discard")
    result = {
        "id": int(item["id"]),
        "connector_id": item.get("connector_id"),
        ...
        "rate_limit_value_raw": item.get("rate_limit_value_raw") or raw_payload.get("rate_limit_value_raw") or "",
        "rate_limit_unit": item.get("rate_limit_unit") or rate_limit_unit_for_bps(item.get("rate_limit_bps")),
        ...
        "policy_decision": item.get("policy_decision") or "",
        "policy_severity": item.get("policy_severity") or "",
        "policy_reasons": bgp_json_loads(item.get("policy_reasons"), []),
...
def bgp_connector_payload_to_values(payload: BgpConnectorPayload) -> dict[str, Any]:
    ...
    backend_type = clean_text(payload.backend or payload.backend_type or "dry_run")
    return {
        "name": name,
        "role": normalize_choice(payload.role, BGP_CONNECTOR_ROLES, "role"),
        "backend_type": normalize_choice(backend_type, BGP_CONNECTOR_BACKENDS, "backend"),
        ...
        "local_address": optional_ip(payload.local_address, "local_address") if clean_text(payload.local_address) else "",
        "router_id": optional_ip(payload.router_id, "router_id") if clean_text(payload.router_id) else "",
        "passive_listen_enabled": 1 if payload.passive_listen_enabled else 0,
        "listen_port": int(payload.listen_port or 179),
        ...
        "exabgp_config_path": clean_text(payload.exabgp_config_path),
        "exabgp_pipe_in": clean_text(payload.exabgp_pipe_in),
        "exabgp_pipe_out": clean_text(payload.exabgp_pipe_out),
        "systemd_service_name": clean_text(payload.systemd_service_name),
        "router_check_enabled": 1 if payload.router_check_enabled else 0,
        "router_vendor": clean_text(payload.router_vendor) or "huawei_vrp",
        "router_mgmt_ip": optional_ip(payload.router_mgmt_ip, "router_mgmt_ip") if clean_text(payload.router_mgmt_ip) else "",
        "router_username": clean_text(payload.router_username),
        "router_auth_secret_ref": clean_text(payload.router_auth_secret_ref),
        "router_password": clean_text(payload.router_password),
...
def bgp_profile_payload_to_values(payload: BgpResponseProfilePayload) -> dict[str, Any]:
    ...
    tcp_flags_value = clean_text(payload.tcp_flags_value).lower() if tcp_flags_selector == "fixed" else ""
    ...
    max_rate = payload.max_rate_limit_bps
    if default_action == "rate_limit" and not default_rate_bps:
    ...
    return {
        ...
        "tcp_flags_selector": tcp_flags_selector,
        "tcp_flags_value": tcp_flags_value,
        "rate_limit_bps": default_rate_bps,
        "rate_limit_value_raw": default_rate_raw,
        "rate_limit_unit": rate_limit_unit_for_bps(default_rate_bps),
        "default_rate_limit_bps": default_rate_bps,
...
def bgp_port_policy_row_to_dict(row: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "port_kind": item.get("port_kind") or "source",
        "port_value": int(item.get("port_value") or 0),
        "name": item.get("name") or "",
        "is_critical": sqlite_bool(item.get("is_critical")),
        "allow_port_only_auto": sqlite_bool(item.get("allow_port_only_auto")),
        "require_ip_scope": sqlite_bool(item.get("require_ip_scope")),
        "allow_with_dst_protected_prefix": sqlite_bool(item.get("allow_with_dst_protected_prefix")),
        "allow_with_src_ip": sqlite_bool(item.get("allow_with_src_ip")),
        "allow_with_dst_ip": sqlite_bool(item.get("allow_with_dst_ip")),
        "min_dst_prefixlen_auto": item.get("min_dst_prefixlen_auto"),
        "min_src_prefixlen_auto": item.get("min_src_prefixlen_auto"),
        "description": item.get("description") or "",
        "is_active": sqlite_bool(item.get("is_active")),
        "created_at": item["created_at"],
        "updated_at": item["updated_at"],
    }


def bgp_port_policy_payload_to_values(payload: BgpMitigationPortPolicyPayload) -> dict[str, Any]:
    protocol = normalize_bgp_protocol(payload.protocol) or "any"
    if protocol not in {"any", "tcp", "udp", "icmp"}:
        raise HTTPException(status_code=400, detail="protocol invalido")
    port_kind = normalize_choice(payload.port_kind, {"source", "destination", "both"}, "port_kind")
    name = clean_text(payload.name)
    if not name:
        raise HTTPException(status_code=400, detail="name obrigatorio")
    return {
        "protocol": protocol,
        "port_kind": port_kind,
        "port_value": int(payload.port_value),
        "name": name,
        "is_critical": 1 if payload.is_critical else 0,
        "allow_port_only_auto": 1 if payload.allow_port_only_auto else 0,
        "require_ip_scope": 1 if payload.require_ip_scope else 0,
        "allow_with_dst_protected_prefix": 1 if payload.allow_with_dst_protected_prefix else 0,
        "allow_with_src_ip": 1 if payload.allow_with_src_ip else 0,
        "allow_with_dst_ip": 1 if payload.allow_with_dst_ip else 0,
        "min_dst_prefixlen_auto": payload.min_dst_prefixlen_auto,
        "min_src_prefixlen_auto": payload.min_src_prefixlen_auto,
        "description": clean_text(payload.description),
        "is_active": 1 if payload.is_active else 0,
    }


def bgp_run_command(args: list[str], timeout: float = 2.0) -> tuple[int, str]:
    try:
...
def normalize_flowspec_port(value: Any, field_name: str) -> str:
    ...
    if not text:
        return ""
    return normalize_bgp_port_text(text, field_name)


def parse_rate_limit_to_bps(value: str) -> int:
    text = clean_text(value).replace(" ", "")
    if not text:
        raise ValueError("rate-limit vazio")
    match = re.fullmatch(r"([+-]?\d+)([kKmMgG]?)(?:bps|bit)?", text, flags=re.IGNORECASE)
    if not match:
        raise ValueError("rate-limit invalido")
...
def normalize_flowspec_action(value: Any, fallback: str = "discard") -> str:
    text = clean_text(value).lower().replace("-", "_")
    if not text:
        text = fallback
    if text not in BGP_FLOWSPEC_ACTIONS:
        raise HTTPException(status_code=400, detail="action FlowSpec invalida")
    return text


def rate_limit_unit_for_bps(value: int | None) -> str:
    bps = int(value or 0)
    if bps and bps % 1_000_000_000 == 0:
        return "G"
    if bps and bps % 1_000_000 == 0:
        return "M"
    if bps and bps % 1_000 == 0:
        return "K"
    return "bps" if bps else ""


def normalize_rate_limit_bps(raw: Any = None, bps: Any = None) -> int | None:
    if clean_text(raw):
        try:
            return parse_rate_limit_to_bps(clean_text(raw))
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from None
    if bps is None or clean_text(bps) == "":
        return None
    try:
        value = int(bps)
    except (TypeError, ValueError):
        raise HTTPException(status_code=400, detail="rate_limit_bps invalido") from None
    if value <= 0:
        raise HTTPException(status_code=400, detail="rate_limit_bps deve ser maior que zero")
    return value
...
def flowspec_action_settings(candidate: dict[str, Any], profile: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "action": action,
        "then_action": action,
        "rate_limit_bps": bps if action == "rate_limit" else None,
        "rate_limit_value_raw": raw if action == "rate_limit" else "",
        "rate_limit_unit": rate_limit_unit_for_bps(bps) if action == "rate_limit" else "",
    }


def render_flowspec_then_clause(action: Any, rate_limit_bps: int | None = None) -> str:
    normalized = normalize_flowspec_action(action)
    if normalized == "discard":
        return "then { discard; }"
    if normalized == "accept":
        return "then { accept; }"
    bps = normalize_rate_limit_bps(None, rate_limit_bps)
    if not bps:
        raise HTTPException(status_code=400, detail="rate_limit_bps obrigatorio para action=rate_limit")
    return f"then {{ rate-limit {bps}; }}"


def flowspec_candidate_from_payload(payload: BgpFlowspecTestPayload) -> dict[str, Any]:
    action = normalize_choice(clean_text(payload.action) or "dry_run", {"announce", "withdraw", "dry_run"}, "action")
    settings = flowspec_action_settings(
        {
            "action": payload.then_action,
            "rate_limit_value_raw": payload.rate_limit_value_raw,
            "rate_limit_bps": payload.rate_limit_bps,
        }
    )
    src_cidr = normalize_bgp_host_or_cidr(payload.src_cidr, "src_cidr") if clean_text(payload.src_cidr) else ""
    dst_cidr = normalize_bgp_host_or_cidr(payload.dst_cidr, "dst_cidr") if clean_text(payload.dst_cidr) else ""
    ...
    return {
        ...
        "then_action": settings["then_action"],
        "rate_limit_bps": settings["rate_limit_bps"],
        "rate_limit_value_raw": settings["rate_limit_value_raw"],
        "rate_limit_unit": settings["rate_limit_unit"],
        "target_prefix": dst_cidr or src_cidr,
        "src_prefix": src_cidr,
        "dst_prefix": dst_cidr,
        "protocol": protocol,
...
def render_exabgp_flowspec_command(command_action: str, candidate: dict[str, Any]) -> str:
    ...
    if candidate.get("dst_port"):
        match_parts.append(f"destination-port ={candidate['dst_port']};")
    if candidate.get("tcp_flags"):
        match_parts.append(f"tcp-flags {clean_text(candidate['tcp_flags']).replace('_', '+')};")
    then_clause = render_flowspec_then_clause(candidate.get("action") or candidate.get("then_action") or "discard", candidate.get("rate_limit_bps"))
    return f"{command_action} flow route {{ match {{ {' '.join(match_parts)} }} {then_clause} }}"


def flowspec_candidate_from_announcement(item: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    row = dict(item)
    raw_payload = bgp_json_loads(row.get("raw_payload"), {})
    action = row.get("action") or raw_payload.get("then_action") or raw_payload.get("action") or "discard"
    if not action and "discard" in clean_text(row.get("rendered_command")).lower():
        action = "discard"
    return {
        "src_prefix": row.get("src_prefix") or "",
        "dst_prefix": row.get("dst_prefix") or "",
        "target_prefix": row.get("target_prefix") or "",
        "protocol": row.get("protocol") or "",
        "src_port": row.get("src_port") or "",
        "dst_port": row.get("dst_port") or "",
        "tcp_flags": row.get("tcp_flags") or "",
        "action": normalize_flowspec_action(action or "discard"),
        "rate_limit_bps": row.get("rate_limit_bps") or raw_payload.get("rate_limit_bps"),
        "rate_limit_value_raw": row.get("rate_limit_value_raw") or raw_payload.get("rate_limit_value_raw") or "",
    }
...
def exabgp_write_pipe(connector: dict[str, Any], command: str) -> None:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Pipe ExaBGP nao encontrado: {pipe_path}") from None
    except BlockingIOError:
        raise HTTPException(status_code=503, detail="Pipe ExaBGP sem leitor ativo") from None
    except OSError as exc:
        raise HTTPException(status_code=500, detail=f"Falha ao escrever no pipe ExaBGP: {exc}") from None


def is_documentation_prefix(prefix: str) -> bool:
    try:
        network = ip_network(prefix, strict=False)
    except ValueError:
        return False
    docs = [ip_network("192.0.2.0/24"), ip_network("198.51.100.0/24"), ip_network("203.0.113.0/24")]
    return network.version == 4 and any(network.subnet_of(doc) or network.overlaps(doc) for doc in docs)


def protected_prefix_match(conn: sqlite3.Connection, prefix: str) -> dict[str, Any] | None:
    if not clean_text(prefix):
        return None
    for row in conn.execute("SELECT * FROM bgp_protected_prefixes WHERE enabled = 1").fetchall():
        item = bgp_protected_prefix_row_to_dict(row)
        if bgp_prefix_overlaps(prefix, item["cidr"]):
            return item
    return None
...
def evaluate_mitigation_policy(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    protocol = clean_text(candidate.get("protocol")) or "any"
    ...
    rate_limit_bps = normalize_rate_limit_bps(candidate.get("rate_limit_value_raw"), candidate.get("rate_limit_bps")) if action == "rate_limit" else None
    requested_mode = clean_text(candidate.get("requested_mode")) or clean_text(candidate.get("mitigation_mode")) or "manual_approval"
    reasons: list[str] = []
    warnings: list[str] = []
    ...
    for prefix in scoped_prefixes:
        try:
            ...
            pass
    if protocol == "any" and not has_scope:
        decision = "deny"
        severity = "danger"
        reasons.append("DENY: protocol any sem src_cidr e sem dst_cidr.")
    if requested_mode == "automatic" and action == "accept":
        decision = "deny"
        severity = "danger"
        reasons.append("DENY: action accept nao e permitida em mitigacao automatica.")
    if action == "rate_limit" and not rate_limit_bps:
        decision = "deny"
        severity = "danger"
        reasons.append("DENY: rate_limit exige rate_limit_bps valido.")
    if requested_mode == "automatic" and action == "rate_limit":
        max_rate = candidate.get("max_rate_limit_bps")
        min_rate = candidate.get("min_rate_limit_bps")
        if max_rate is not None and rate_limit_bps and rate_limit_bps > int(max_rate):
            decision = "require_manual_approval" if decision != "deny" else decision
            severity = "caution" if severity != "danger" else severity
            reasons.append(f"rate-limit {rate_limit_bps} bps acima do maximo automatico {int(max_rate)} bps.")
        if min_rate is not None and rate_limit_bps and rate_limit_bps < int(min_rate):
            decision = "require_manual_approval" if decision != "deny" else decision
            severity = "caution" if severity != "danger" else severity
            reasons.append(f"rate-limit {rate_limit_bps} bps abaixo do minimo automatico {int(min_rate)} bps.")
        if not (has_host_scope or dst_protected or src_protected):
            decision = "require_manual_approval" if decision != "deny" else decision
            severity = "caution" if severity != "danger" else severity
            reasons.append("rate-limit automatico exige IP host (/32 ou /128) ou prefixo protegido.")
    if matched and not has_scope:
        decision = "deny"
        severity = "danger"
        reasons.append("DENY: critical whitelisted port requires src_cidr or dst_cidr scope. Port-only automatic block is not allowed.")
        required_scope.extend(["src_cidr", "dst_cidr"])
    for policy in matched:
        port_label = f"{policy['protocol']}/{policy['port_kind']}/{policy['port_value']}"
        if policy["is_critical"] and policy["require_ip_scope"] and not has_scope:
            decision = "deny"
            severity = "danger"
            reasons.append(f"DENY: porta critica {port_label} exige escopo IP/prefixo.")
        if requested_mode == "automatic":
            if policy["port_kind"] == "source" and policy["port_value"] in {53, 123} and not dst_cidr:
                decision = "deny" if not src_cidr else "require_manual_approval"
                severity = "danger" if decision == "deny" else "caution"
                reasons.append(f"{policy['name']}: source-port reflection exige destination protegido para automatico.")
                required_scope.append("dst_cidr")
            if dst_cidr:
                try:
    ...
    if requested_mode != "automatic" and decision != "deny":
        decision = "require_manual_approval"
        severity = "caution"
        reasons.append("Vetor/perfil nao esta em modo automatic.")
    if not reasons and decision == "allow_auto":
        reasons.append("AUTO_ALLOWED: escopo e politica permitem mitigacao automatica.")
    return {
        "decision": decision,
        "severity": severity,
...
def candidate_from_bgp_payload(payload: BgpAnnouncementDryRunPayload, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if selector in {"src_ip", "anomaly_src_ip"} and payload.src_ip:
        ...
        if payload.dst_ip:
            dst_prefix = normalize_bgp_host_or_cidr(payload.dst_ip, "dst_ip")
    elif selector in {"dst_prefix"} and payload.dst_ip:
        dst_prefix = normalize_bgp_host_or_cidr(payload.dst_ip, "dst_ip")
    elif selector in {"src_prefix"} and payload.src_ip:
        src_prefix = normalize_bgp_host_or_cidr(payload.src_ip, "src_ip")
    elif selector in {"conversation"}:
        if payload.src_ip:
            src_prefix = normalize_bgp_host_or_cidr(payload.src_ip, "src_ip")
        if payload.dst_ip:
            dst_prefix = normalize_bgp_host_or_cidr(payload.dst_ip, "dst_ip")
    ...
    elif protocol_selector == "fixed":
        protocol = normalize_bgp_protocol(profile.get("fixed_protocol"))
    ...
    action_settings = flowspec_action_settings(
        {
            "action": payload.action or profile.get("default_action") or profile.get("action"),
            "rate_limit_value_raw": payload.rate_limit_value_raw,
            "rate_limit_bps": payload.rate_limit_bps,
        },
        profile,
    ...
    return {
        ...
        "then_action": action_settings["then_action"],
        "rate_limit_bps": action_settings["rate_limit_bps"],
        "rate_limit_value_raw": action_settings["rate_limit_value_raw"],
        "rate_limit_unit": action_settings["rate_limit_unit"],
...
def response_profile_candidate_for_anomaly(
    profile: dict[str, Any],
    anomaly: dict[str, Any],
    flow_context: dict[str, Any],
) -> dict[str, Any]:
    dominant_dst_ip = clean_ip(flow_context.get("dominant_dst_ip")) or clean_ip(anomaly.get("top_dst_ip") or anomaly.get("dst_ip") or anomaly.get("target_ip"))
    dominant_dst_port = int(flow_context.get("dominant_dst_port") or anomaly.get("top_dst_port") or anomaly.get("target_port") or 0)
    dominant_protocol = clean_text(flow_context.get("dominant_protocol") or anomaly.get("protocol") or anomaly.get("decoder")).lower()
    src_ip = clean_ip(anomaly.get("top_src_ip") or anomaly.get("src_ip"))
    top_conversations = flow_context.get("top_conversations_by_packets") if isinstance(flow_context.get("top_conversations_by_packets"), list) else []
    if not src_ip and top_conversations:
        src_ip = clean_ip(top_conversations[0].get("src_ip"))
    if not dominant_dst_ip and top_conversations:
        dominant_dst_ip = clean_ip(top_conversations[0].get("dst_ip"))
    if not dominant_dst_port and top_conversations:
        dominant_dst_port = int(top_conversations[0].get("dst_port") or 0)

    selector = clean_text(profile.get("target_selector") or "dst_ip")
    src_prefix = ""
    dst_prefix = ""
    target_prefix = ""
    if selector in {"src_ip", "anomaly_src_ip", "src_prefix"} and src_ip:
        src_prefix = normalize_bgp_host_or_cidr(src_ip, "src_ip")
    if selector in {"dst_ip", "anomaly_dst_ip", "dst_prefix"} and dominant_dst_ip:
        dst_prefix = normalize_bgp_host_or_cidr(dominant_dst_ip, "dst_ip")
    if selector in {"src_and_dst_ip", "conversation"}:
        if src_ip:
            src_prefix = normalize_bgp_host_or_cidr(src_ip, "src_ip")
        if dominant_dst_ip:
            dst_prefix = normalize_bgp_host_or_cidr(dominant_dst_ip, "dst_ip")
    if selector in {"target_ip", "target_cidr"}:
        target = anomaly.get("target_cidr") or anomaly.get("target_ip")
        if target:
            target_prefix = normalize_bgp_host_or_cidr(target, "target")
    target_prefix = target_prefix or dst_prefix or src_prefix

    protocol_selector = clean_text(profile.get("protocol_selector"))
    if protocol_selector == "fixed":
        protocol = clean_text(profile.get("fixed_protocol"))
    elif protocol_selector in {"tcp", "udp", "icmp"}:
        protocol = protocol_selector
    elif protocol_selector in {"manual", "anomaly_protocol"}:
        protocol = normalize_bgp_protocol(dominant_protocol)
    else:
        protocol = ""
    src_port = resolve_bgp_port(profile.get("src_port_selector") or "any", profile.get("src_port_value") or "", anomaly.get("top_src_port") or anomaly.get("src_port"))
    dst_port = resolve_bgp_port(profile.get("dst_port_selector") or "any", profile.get("dst_port_value") or "", dominant_dst_port)
    action_settings = flowspec_action_settings({"action": profile.get("default_action") or profile.get("action")}, profile)
    candidate = {
        "response_profile_id": profile["id"],
        "response_profile_name": profile.get("name") or "",
        "connector_id": profile.get("connector_id"),
        "connector_name": profile.get("connector_name") or "",
        "response_type": "flowspec" if profile.get("response_type") == "rate_limit" else "rtbh" if profile.get("response_type") == "blackhole" else profile.get("response_type"),
        "action": "rate_limit" if profile.get("response_type") == "rate_limit" else action_settings["action"],
        "then_action": "rate_limit" if profile.get("response_type") == "rate_limit" else action_settings["then_action"],
        "rate_limit_bps": action_settings["rate_limit_bps"],
        "rate_limit_value_raw": action_settings["rate_limit_value_raw"],
        "rate_limit_unit": action_settings["rate_limit_unit"],
        ...
        "src_port": src_port,
        "dst_port": dst_port,
        "tcp_flags": clean_text(profile.get("tcp_flags_value") if profile.get("tcp_flags_selector") == "fixed" else profile.get("tcp_flags_selector") if profile.get("tcp_flags_selector") in {"syn", "syn_ack"} else ""),
        "duration_seconds": int(profile.get("default_duration_seconds") or 1800),
    ...
    try:
        ...
        candidate["rendered_command_preview"] = f"INVALID: {exc}"
    candidate["rendered_match"] = {
        "source": candidate["src_prefix"],
        "destination": candidate["dst_prefix"] or candidate["target_prefix"],
        "protocol": candidate["protocol"],
        "source_port": candidate["src_port"],
        "destination_port": candidate["dst_port"],
        "tcp_flags": candidate["tcp_flags"],
    }
    candidate["rendered_then"] = {"action": candidate["action"], "rate_limit_bps": candidate.get("rate_limit_bps")}
    return candidate


def validate_response_profile_for_anomaly(
    profile: dict[str, Any],
    anomaly: dict[str, Any],
    flow_context: dict[str, Any],
    ...
    errors: list[str] = []
    warnings: list[str] = []
    status = response_profile_status(profile)
    if status != "valid":
        errors.append(status)
    evidence_status = clean_text(flow_context.get("evidence_status") or candidate.get("evidence_status"))
    if evidence_status == "insufficient":
        errors.append("evidence_status_insufficient")
    direction = clean_text(anomaly.get("direction")).lower()
    outbound = direction in {"sends", "transmits", "outbound"}
    if outbound and candidate.get("src_prefix") and not candidate.get("dst_prefix"):
        errors.append("unsafe_target_src_only_outbound")
    if clean_text(profile.get("dst_port_selector")) == "anomaly_dst_port" and not clean_text(candidate.get("dst_port")):
        errors.append("missing_anomaly_dst_port")
    if clean_text(profile.get("protocol_selector")) in {"manual", "anomaly_protocol"} and not clean_text(candidate.get("protocol")):
        errors.append("missing_anomaly_protocol")
    if not clean_text(candidate.get("target_prefix") or candidate.get("src_prefix") or candidate.get("dst_prefix")):
        errors.append("target_empty")
    if connector is None and profile.get("connector_id"):
...
def validate_mitigation_candidate(candidate: dict[str, Any], connector: dict[str, Any] | None, response_profile: dict[str, Any]) -> dict[str, list[str]]:
    ...
    if candidate.get("scope_type") == "internal_ip_32":
        ...
        if not target_ip:
            errors.append("invalid_scope: anomalia internal_ip_32 sem target_ip.")
        if not target_cidr or not is_ipv4_32_cidr(target_cidr):
            errors.append("invalid_scope: target_cidr de internal_ip_32 deve ser /32.")
        elif target_ip and host_cidr_for_ip(target_ip) != target_cidr:
            errors.append("invalid_scope: target_cidr nao corresponde ao target_ip.")
    ...
    if response_type == "flowspec":
        ...
        if action == "rate_limit":
            if not rate_limit_bps:
                errors.append("FlowSpec rate-limit exige limite maior que zero.")
            min_rate = response_profile.get("min_rate_limit_bps")
            max_rate = response_profile.get("max_rate_limit_bps")
            if rate_limit_bps and min_rate is not None and rate_limit_bps < int(min_rate):
                errors.append(f"rate-limit abaixo do minimo permitido ({int(min_rate)} bps).")
            if rate_limit_bps and max_rate is not None and rate_limit_bps > int(max_rate):
                errors.append(f"rate-limit acima do maximo permitido ({int(max_rate)} bps).")
        if action in {"discard", "rate_limit"}:
            for prefix in prefixes:
                try:
                    network = ip_network(prefix, strict=False)
        ...
        duplicate = conn.execute(
            ...
            (
                *active_statuses,
                ...
                int(candidate.get("rate_limit_bps") or 0),
...
def render_bgp_announcement(candidate: dict[str, Any], connector: dict[str, Any], response_profile: dict[str, Any]) -> str:
    ...
    if response_type == "flowspec":
        return render_exabgp_flowspec_command("announce", candidate)
    return render_exabgp_flowspec_command("announce", {**candidate, "action": action or "discard"})
...
def create_bgp_announcement(conn: sqlite3.Connection, candidate: dict[str, Any], connector: dict[str, Any], profile: dict[str, Any], validation: dict[str, list[str]], created_by: str, anomaly_id: int | None = None) -> dict[str, Any]:
    ...
    policy = candidate.get("policy") or evaluate_mitigation_policy(
        {
            **candidate,
            "requested_mode": profile.get("approval_mode") or connector.get("mode") or "manual_approval",
...
def protocol_from_flow_or_event(flow: dict[str, Any], event: dict[str, Any]) -> str:
    proto = clean_text(flow.get("proto_name") or flow.get("protocol") or event.get("protocol") or proto_name(flow.get("proto"))).lower()
...
def attach_mitigation_config(conn: sqlite3.Connection, candidate: dict[str, Any], vector: dict[str, Any] | None) -> dict[str, Any]:
    ...
    action_settings = flowspec_action_settings(candidate, profile)
    ...
    with sqlite_connection() as conn:
        if event_vector in UDP_UPLOAD_MANY_CLIENTS_ALIASES or event.get("scope_type") == "external_dst_ip_port":
            ...
            if protocol == "udp" and event_target and event_port:
                ...
                candidate.update(
                    {
                        "dst_cidr": event_target,
                        "protocol": "udp",
                        "dst_port": str(event_port),
                        "target_scope": event_target,
                        "target_ip": clean_ip(event.get("target_ip")) or dst_ip,
                        "target_cidr": event_target,
...
def evaluated_mitigation_candidates(anomaly_id: int) -> dict[str, Any]:
    ...
    for candidate in candidates:
        ...
        try:
            ...
            rendered = f"INVALID: {clean_text(exc.detail)}"
...
def flow_context_from_analysis(analysis: dict[str, Any]) -> dict[str, Any]:
    rows = [dict(row) for row in analysis.get("top_flows") or [] if isinstance(row, dict)]

    def top_ip(field: str, metric: str) -> list[dict[str, Any]]:
        totals: dict[str, dict[str, Any]] = {}
        for row in rows:
            key = clean_ip(row.get(field))
            if not key:
                continue
            item = totals.setdefault(key, {"ip": key, "packets": 0, "bytes": 0, "flow_count": 0})
            item["packets"] += int(row.get("packets") or 0)
            item["bytes"] += int(row.get("bytes") or 0)
            item["flow_count"] += int(row.get("flow_count") or 1)
        return sorted(totals.values(), key=lambda item: (int(item.get(metric) or 0), int(item.get("packets") or 0)), reverse=True)[:20]

    def top_port(field: str, metric: str) -> list[dict[str, Any]]:
        totals: dict[str, dict[str, Any]] = {}
        for row in rows:
            try:
                port = int(row.get(field) or 0)
            except (TypeError, ValueError):
                port = 0
            if port <= 0:
                continue
            key = str(port)
            item = totals.setdefault(key, {"port": port, "packets": 0, "bytes": 0, "flow_count": 0})
            item["packets"] += int(row.get("packets") or 0)
            item["bytes"] += int(row.get("bytes") or 0)
            item["flow_count"] += int(row.get("flow_count") or 1)
        return sorted(totals.values(), key=lambda item: (int(item.get(metric) or 0), int(item.get("packets") or 0)), reverse=True)[:20]
    ...
    for row in rows:
        conversations.append(
            {
                **row,
                "conversation": f"{clean_ip(row.get('src_ip'))}:{int(row.get('src_port') or 0)} -> {clean_ip(row.get('dst_ip'))}:{int(row.get('dst_port') or 0)} {clean_text(row.get('protocol') or row.get('proto_name') or row.get('proto')).upper()}",
            }
        )
    by_packets = sorted(conversations, key=lambda item: (int(item.get("packets") or 0), int(item.get("bytes") or 0)), reverse=True)[:20]
    by_bytes = sorted(conversations, key=lambda item: (int(item.get("bytes") or 0), int(item.get("packets") or 0)), reverse=True)[:20]
    dominant = analysis.get("top_dst_port_pair_by_packets") or {}
    top_flow = analysis.get("top_flow_by_packets") or {}
    evidence_status = analysis.get("evidence_status") or ("insufficient" if not rows else "weak")
    attack_shape = "unknown"
    if evidence_status != "insufficient" and dominant:
        attack_shape = "multi_source_same_dst_port" if int(dominant.get("unique_src_ips") or 0) >= 2 else "single_source_single_dst_port"
    elif evidence_status == "insufficient":
        attack_shape = "insufficient_flow_evidence"
    return {
        "anomaly_id": analysis.get("anomaly_id"),
        "top_src_ips_by_packets": top_ip("src_ip", "packets"),
        "top_src_ips_by_bytes": top_ip("src_ip", "bytes"),
        "top_dst_ips_by_packets": top_ip("dst_ip", "packets"),
        "top_dst_ips_by_bytes": top_ip("dst_ip", "bytes"),
        "top_dst_ports_by_packets": top_port("dst_port", "packets"),
        "top_dst_ports_by_bytes": top_port("dst_port", "bytes"),
        "top_src_ports_by_packets": top_port("src_port", "packets"),
        "top_conversations_by_packets": by_packets,
        "top_conversations_by_bytes": by_bytes,
        "related_flows": rows[:50],
        "unique_src_ips": int(analysis.get("unique_src_ips") or len({clean_ip(row.get("src_ip")) for row in rows if clean_ip(row.get("src_ip"))})),
        "unique_dst_ips": int(analysis.get("unique_dst_ips") or len({clean_ip(row.get("dst_ip")) for row in rows if clean_ip(row.get("dst_ip"))})),
        "unique_dst_ports": int(analysis.get("unique_dst_ports") or len({int(row.get("dst_port") or 0) for row in rows if int(row.get("dst_port") or 0)})),
        "dominant_dst_ip": clean_ip(top_flow.get("dst_ip")) or clean_text(dominant.get("dst_ip")),
        "dominant_dst_port": int(top_flow.get("dst_port") or 0) or int(dominant.get("dst_port") or 0),
        "dominant_protocol": clean_text(top_flow.get("protocol") or top_flow.get("proto_name") or dominant.get("protocol")).lower(),
        "attack_shape": attack_shape,
        "evidence_status": evidence_status,
        "warnings": analysis.get("warnings") or ([] if rows else ["Sem flows detalhados encontrados para esta janela."]),
    }
...
def compact_flow_context_for_ai(flow_context: dict[str, Any]) -> dict[str, Any]:
    if not flow_context:
        return {}
    keys = (
        "evidence_status",
    ...
    compacted = {key: flow_context.get(key) for key in keys if ai_keep_value(flow_context.get(key))}
    for key in (
        "top_src_ips_by_packets",
        "top_dst_ips_by_packets",
        "top_dst_ports_by_packets",
        "top_conversations_by_packets",
        "related_flows",
    ):
        values = flow_context.get(key)
        if isinstance(values, list) and values:
            compacted[key] = values[:5]
    return compacted
...
@app.get("/api/anomalies/{event_id}/flow-context")
...
def create_bgp_connector(request: Request, payload: BgpConnectorPayload):
    ...
    now = utc_now_iso()
    columns = (
        "name", "role", "backend_type", "mode", "local_asn", "peer_asn", "peer_ip",
        "local_address", "router_id", "passive_listen_enabled", "listen_port",
        "default_next_hop", "default_community", "default_large_community",
        "exabgp_config_path", "exabgp_pipe_in", "exabgp_pipe_out", "systemd_service_name",
        "router_check_enabled", "router_vendor", "router_mgmt_ip", "router_username",
        "router_auth_secret_ref", "router_password", "max_active_rules", "max_duration_seconds",
...
@app.get("/api/bgp/connectors/{connector_id}/status")
def get_bgp_connector_status(request: Request, connector_id: int):
    require_admin(request)
    ensure_sensor_db()
    with sqlite_connection() as conn:
...
def test_bgp_connector_flowspec(request: Request, connector_id: int, payload: BgpFlowspecTestPayload):
    require_admin(request)
    ensure_sensor_db()
    payload_action = clean_text(payload.action) or "dry_run"
    ...
    with sqlite_connection() as conn:
        ...
        profile = {
            ...
            "default_action": payload_then_action,
            "rate_limit_bps": payload.rate_limit_bps,
            "rate_limit_value_raw": payload.rate_limit_value_raw or "",
            "default_rate_limit_bps": normalize_rate_limit_bps(payload.rate_limit_value_raw, payload.rate_limit_bps) if payload_then_action == "rate_limit" else None,
            "default_rate_limit_raw": payload.rate_limit_value_raw or "",
            "max_rate_limit_bps": None,
            "min_rate_limit_bps": None,
            "max_duration_seconds": connector.get("max_duration_seconds") or BGP_DEFAULT_MAX_DURATION_SECONDS,
            "default_duration_seconds": payload.duration_seconds or 1800,
            "allow_wide_prefix": False,
            "require_protocol_or_port": True,
        }
...
@app.post("/api/bgp/response-profiles/{profile_id}/test-anomaly")
def test_bgp_response_profile_with_anomaly(request: Request, profile_id: int, payload: dict[str, Any]):
    require_admin(request)
    ensure_sensor_db()
    ...
    candidate.update(
        {
            ...
            "allow_auto": validation["allow_auto"],
            "apply_enabled": validation["apply_enabled"],
        }
    )
    return {
        "anomaly": anomaly,
...
@app.get("/api/bgp/port-policies")
def list_bgp_port_policies(request: Request, include_disabled: bool = True):
    require_admin(request)
    ensure_sensor_db()
    where = "" if include_disabled else "WHERE is_active = 1"
    with sqlite_connection() as conn:
        rows = conn.execute(f"SELECT * FROM bgp_mitigation_port_policies {where} ORDER BY protocol, port_kind, port_value, id").fetchall()
    return {"items": [bgp_port_policy_row_to_dict(row) for row in rows]}


@app.post("/api/bgp/port-policies", status_code=201)
def create_bgp_port_policy(request: Request, payload: BgpMitigationPortPolicyPayload):
    require_admin(request)
    ensure_sensor_db()
    values = bgp_port_policy_payload_to_values(payload)
    columns = (
        "protocol", "port_kind", "port_value", "name", "is_critical", "allow_port_only_auto",
        "require_ip_scope", "allow_with_dst_protected_prefix", "allow_with_src_ip", "allow_with_dst_ip",
        "min_dst_prefixlen_auto", "min_src_prefixlen_auto", "description", "is_active",
    )
    now = utc_now_iso()
    with sqlite_connection() as conn:
        cursor = conn.execute(
            f"INSERT INTO bgp_mitigation_port_policies ({', '.join(columns)}, created_at, updated_at) VALUES ({', '.join('?' for _ in columns)}, ?, ?)",
            tuple(values[column] for column in columns) + (now, now),
        )
        conn.commit()
        row = conn.execute("SELECT * FROM bgp_mitigation_port_policies WHERE id = ?", (int(cursor.lastrowid),)).fetchone()
        return bgp_port_policy_row_to_dict(row)


@app.put("/api/bgp/port-policies/{policy_id}")
def update_bgp_port_policy(request: Request, policy_id: int, payload: BgpMitigationPortPolicyPayload):
    require_admin(request)
    ensure_sensor_db()
    values = bgp_port_policy_payload_to_values(payload)
    columns = (
    ...
    with sqlite_connection() as conn:
        row = conn.execute("SELECT id FROM bgp_mitigation_port_policies WHERE id = ?", (policy_id,)).fetchone()
        if row is None:
            raise HTTPException(status_code=404, detail="Politica de porta nao encontrada")
        conn.execute(
            f"UPDATE bgp_mitigation_port_policies SET {', '.join(f'{column} = ?' for column in columns)}, updated_at = ? WHERE id = ?",
            tuple(values[column] for column in columns) + (utc_now_iso(), policy_id),
        )
        conn.commit()
        row = conn.execute("SELECT * FROM bgp_mitigation_port_policies WHERE id = ?", (policy_id,)).fetchone()
        return bgp_port_policy_row_to_dict(row)


@app.delete("/api/bgp/port-policies/{policy_id}")
def delete_bgp_port_policy(request: Request, policy_id: int):
    require_admin(request)
    ensure_sensor_db()
    with sqlite_connection() as conn:
        row = conn.execute("SELECT id FROM bgp_mitigation_port_policies WHERE id = ?", (policy_id,)).fetchone()
        if row is None:
            raise HTTPException(status_code=404, detail="Politica de porta nao encontrada")
        conn.execute("UPDATE bgp_mitigation_port_policies SET is_active = 0, updated_at = ? WHERE id = ?", (utc_now_iso(), policy_id))
        conn.commit()
    return {"ok": True, "disabled": True}
...
def bgp_report_pdf(request: Request):
    ...
    ensure_sensor_db()
    with sqlite_connection() as conn:
        connectors = [bgp_connector_row_to_dict(row) for row in conn.execute("SELECT * FROM bgp_connectors ORDER BY name, id").fetchall()]
        profiles = [bgp_response_profile_row_to_dict(row) for row in conn.execute("SELECT * FROM bgp_response_profiles ORDER BY name, id").fetchall()]
        protected = [bgp_protected_prefix_row_to_dict(row) for row in conn.execute("SELECT * FROM bgp_protected_prefixes ORDER BY cidr, id").fetchall()]
        policies = [bgp_port_policy_row_to_dict(row) for row in conn.execute("SELECT * FROM bgp_mitigation_port_policies ORDER BY protocol, port_kind, port_value, id").fetchall()]
        ...
        announcements = [bgp_announcement_row_to_dict(row) for row in announcement_rows]
    sections = [
        {
            "title": "Resumo",
            "headers": ["Campo", "Valor"],
            "rows": key_value_rows(
                [
                    ("Conectores", len(connectors)),
                    ("Profiles", len(profiles)),
                    ("Prefixos protegidos", len(protected)),
                    ("Politicas de porta", len(policies)),
                    ("Announcements listados", len(announcements)),
                ]
            ),
        },
        {"title": "Conectores", "headers": ["name", "role", "backend_type", "mode", "peer_ip", "enabled"], "rows": connectors[:100]},
        {"title": "Profiles", "headers": ["name", "response_type", "approval_mode", "action", "target_selector", "protocol_selector", "dst_port_selector", "enabled"], "rows": profiles[:100]},
        {"title": "Prefixos protegidos", "headers": ["cidr", "name", "reason", "enabled"], "rows": protected[:100]},
        {"title": "Politicas de porta", "headers": ["name", "protocol", "port_kind", "port_value", "is_critical", "is_active"], "rows": policies[:100]},
        {
            "title": "Announcements",
            "text": "Relatorio limitado aos primeiros 200 registros. Use CSV ou API para extracao completa.",
            "headers": ["id", "status", "connector_name", "response_profile_name", "policy_decision", "action", "target_prefix", "src_prefix", "dst_prefix", "protocol", "dst_port"],
            "rows": announcements,
        },
    ]
    return simple_pdf_response("BGP / Mitigacao", sections, "gmj-bgp-mitigation.pdf")
...
def create_detection_rule(template_id: int, payload: DetectionRulePayload):
    ...
    with sqlite_connection() as conn:
        ...
        validate_detection_rule_profile_refs(conn, data)
        now = utc_now_iso()
        cursor = conn.execute(
            """
            INSERT INTO detection_template_rules (
                ...
                detection_key, group_by, scope, input_if, output_if, sensor_id,
                warning_response_profile_id, critical_response_profile_id,
                fallback_response_profile_id, mitigation_mode, mitigation_enabled,
                max_auto_prefixlen_v4, max_auto_prefixlen_v6, require_protected_prefix,
                duration_seconds, max_active_announcements, min_confidence_for_auto,
                use_global_whitelist, extra_whitelist_ids, bypass_whitelist, notes,
                created_at,
                updated_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ...
            (
                ...
                data["output_if"],
                data["sensor_id"],
                data["warning_response_profile_id"],
                data["critical_response_profile_id"],
                data["fallback_response_profile_id"],
...
def target_role_for_zone_direction(direction: Any) -> str:
    normalized = clean_text(direction).lower()
    if normalized in {"transmits", "sends", "out", "upload", "saindo"}:
        return "src_ip"
    if normalized in {"receives", "in", "download", "entrando"}:
        return "dst_ip"
    return ""


def target_role_for_attack_direction(direction: Any) -> str:
    normalized = clean_text(direction).lower()
    if normalized == "sends":
        return "src_ip"
    if normalized == "receives":
        return "dst_ip"
    return ""


def host_cidr_for_ip(value: Any) -> str:
    text = clean_ip(value)
    if not text:
        return ""
    try:
...
def is_ipv4_32_cidr(value: Any) -> bool:
    text = clean_text(value)
    if not text:
        return False
    try:
        ...
        return False
    return network.version == 4 and network.prefixlen == 32


def internal_ip_scope_fields(
    domain: Any,
    direction: Any,
    target_ip: Any,
    *,
    attack_direction: bool = False,
    fallback_cidr: Any = "",
) -> dict[str, Any]:
    scope_type = "internal_ip_32" if clean_text(domain) == "internal_ip" else ""
    role = target_role_for_attack_direction(direction) if attack_direction else target_role_for_zone_direction(direction)
    clean_target = clean_ip(target_ip)
    target_cidr = host_cidr_for_ip(clean_target) if scope_type == "internal_ip_32" else clean_text(fallback_cidr)
    invalid_scope = bool(scope_type == "internal_ip_32" and (not clean_target or not target_cidr or not is_ipv4_32_cidr(target_cidr)))
    return {
...
def query_detection_rule_candidates(
    ...
    for row in rows_as_dicts(result):
        ...
        scope = internal_ip_scope_fields(rule["domain"], rule["direction"], internal_ip)
        item = {
            "zone_id": int(zone["id"]),
            "zone_name": zone["name"],
...
def security_anomaly_message(candidate: dict[str, Any]) -> str:
    target_ip = candidate.get("target_ip") or candidate.get("src_ip") or candidate.get("internal_ip") or "N/D"
    src_ip = candidate.get("src_ip") or candidate.get("internal_ip") or target_ip
    ...
    if candidate.get("scope_type") == "internal_ip_32":
        direction = "saiu da zona" if candidate.get("target_role") == "src_ip" else "entrou na zona"
        return (
            f"O IP {target_ip}/32, pertencente a zona {candidate.get('zone_name')} e ao prefixo {candidate.get('prefix_cidr')}, "
            f"{direction} com {packets_s} em {protocol}, acima do limite configurado."
        )
...
def upsert_security_anomaly(conn: sqlite3.Connection, candidate: dict[str, Any]) -> str:
    now = utc_now_iso()
    if candidate.get("scope_type") == "internal_ip_32" and not candidate.get("target_cidr"):
        scope = internal_ip_scope_fields(candidate.get("domain"), candidate.get("direction"), candidate.get("internal_ip") or candidate.get("src_ip") or candidate.get("dst_ip"))
    ...
    values = (
        ...
        candidate.get("target_ip") or "",
        candidate.get("target_cidr") or "",
        candidate.get("target_role") or "",
        candidate.get("scope_type") or "",
        1 if candidate.get("invalid_scope") else 0,
        candidate.get("protocol") or "",
...
def security_anomaly_row_to_dict(row: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    ...
    if domain == "internal_ip":
        role = item.get("target_role") or target_role_for_zone_direction(direction)
        if role == "dst_ip":
            fallback_target = item.get("target_ip") or item.get("dst_ip") or item.get("src_ip") or ""
        else:
            fallback_target = item.get("target_ip") or item.get("src_ip") or item.get("dst_ip") or ""
        target_cidr = item.get("target_cidr") or host_cidr_for_ip(fallback_target)
        scope_type = item.get("scope_type") or "internal_ip_32"
        invalid_scope = sqlite_bool(item.get("invalid_scope")) or not bool(clean_ip(fallback_target) and is_ipv4_32_cidr(target_cidr))
    else:
        role = item.get("target_role") or ""
    ...
    return {
        ...
        "target_ip": clean_ip(fallback_target),
        "target_cidr": target_cidr,
        "target_role": role,
        "scope_type": scope_type,
        "invalid_scope": invalid_scope,
...
def anomaly_event_row_to_dict(row: sqlite3.Row | dict[str, Any]) -> dict[str, Any]:
    ...
    domain_type = item.get("attack_domain_type") or item.get("domain_type") or ""
    scope_type = item.get("scope_type") or ("internal_ip_32" if domain_type == "internal_ip" and item.get("target_ip") and item.get("target_cidr") in {"", None} else "")
...
def active_zone_prefix_groups() -> list[dict[str, Any]]:
    ...
    with sqlite_connection() as conn:
        ...
        zone_rows = conn.execute(
            """
            SELECT p.zone_id, p.cidr
            FROM ip_zone_prefixes p
            JOIN ip_zones z ON z.id = p.zone_id
            WHERE p.active = 1
              AND z.active = 1
            ORDER BY p.zone_id, p.id
            """
        ).fetchall()
    cidrs_by_zone: dict[int, list[str]] = {}
    for row in zone_rows:
        cidrs_by_zone.setdefault(int(row["zone_id"]), []).append(row["cidr"])
    groups = []
    for row in rows:
        ...
        groups.append(
            {
                ...
                "prefix_cidr": row["prefix_cidr"],
                "prefix_type": row["prefix_type"] or "client",
                "zone_id": zone_id,
                "zone_name": row["zone_name"] or "",
                "zone_cidrs": cidrs_by_zone.get(zone_id, []),
            }
        )
    return groups
...
def query_udp_many_internal_to_same_dst_port(
    ...
    critical_unique = int(os.getenv("GMJFLOW_UDP_MANY_INTERNAL_CRITICAL_UNIQUE_SRC", "5"))
    row_limit = max(1, int(limit))
    groups = active_zone_prefix_groups()
    vector_source_cidr = clean_text(vector.get("target_cidr") or vector.get("src_cidr"))
    if vector_source_cidr:
        groups = [
            {
                "prefix_id": None,
                "prefix_cidr": vector_source_cidr,
                "prefix_type": "manual",
                "zone_id": None,
                "zone_name": "",
                "zone_cidrs": [vector_source_cidr],
            }
        ]
    if not groups:
        return []
    ...
    for group in groups:
        ...
        rate_expr = clickhouse_sample_rate_expr(
            vector.get("sensor_id"),
            "output",
            int(vector["interface_if_index"]) if vector.get("interface_if_index") is not None else None,
        )
        result = query_clickhouse(
            f"""
            SELECT
            ...
            FROM flow_raw
            WHERE {where}
            GROUP BY target_ip, target_port, sensor, output_if
            HAVING packets_s >= {float(warning_pps)}
               AND unique_src_ips >= {int(warning_unique)}
               AND unique_dst_ips = 1
               AND unique_dst_ports = 1
            ORDER BY packets_s DESC
            LIMIT {{limit:UInt32}}
            """,
            params,
        ...
        for row in rows_as_dicts(result):
            ...
            severity = "critical" if packets_s >= critical_pps and unique_src_ips >= critical_unique else "warning"
            threshold = critical_pps if severity == "critical" else warning_pps
            target_ip = clean_ip(row.get("target_ip"))
            items.append(
                {
                    "target_ip": target_ip,
                    "target_cidr": host_cidr_for_ip(target_ip),
                    "target_role": "dst_ip",
...
def query_udp_special_sample_rows(
    ...
    result = query_clickhouse(
        ...
        FROM flow_raw
        WHERE {where}
        ORDER BY bytes DESC, packets DESC, flow_time DESC
        LIMIT {{limit:UInt32}}
        """,
        params,
    )
...
def flow_matches_anomaly_target(flow: dict[str, Any], event: dict[str, Any]) -> bool:
    ...
    if role == "dst_ip":
        return clean_ip(flow.get("dst_ip")) == target_ip
    return clean_ip(flow.get("src_ip")) == target_ip or clean_ip(flow.get("dst_ip")) == target_ip
...
def security_item_matches_event_target(item: dict[str, Any], event: dict[str, Any]) -> bool:
    ...
    if role == "dst_ip":
        return clean_ip(item.get("dst_ip")) == target_ip or clean_ip(item.get("target_ip")) == target_ip
    return clean_ip(item.get("target_ip")) == target_ip
...
def attack_vector_test_result(
    ...
    sample_rows = query_vector_sample_rows(
        vector,
        start_dt,
        end_dt,
        target_ip,
...
def save_anomaly_flow_samples(
    ...
    for row in rows:
        conn.execute(
            ...
            (
                event_id,
                iso(row["flow_time"]) if isinstance(row.get("flow_time"), datetime) else clean_text(row.get("flow_time")),
                row.get("sensor"),
...
def upsert_anomaly_event(
    ...
    scope = internal_ip_scope_fields(
        vector.get("domain_type"),
        vector.get("direction"),
        target_ip,
        attack_direction=True,
        fallback_cidr=vector.get("target_cidr"),
    )
    if clean_text(traffic.get("scope_type")):
        scope = {
            "target_ip": clean_ip(traffic.get("target_ip")),
            "target_cidr": clean_text(traffic.get("target_cidr")),
            "target_role": clean_text(traffic.get("target_role")),
            "scope_type": clean_text(traffic.get("scope_type")),
            "invalid_scope": bool(traffic.get("invalid_scope")),
        }
    observed = metric_value_from_row(traffic, vector["threshold_unit"])
    threshold = float(traffic.get("threshold_value") or vector["threshold_value"] or 0)
...
def attack_vector_test_pdf_response(payload: dict[str, Any]) -> Response:
    rows = list(payload.get("sample_rows") or [])[:100]
    sections = [
        {
            ...
            "rows": key_value_rows(
                [
                    ("Vetor", payload.get("vector_id")),
                    ("Decoder", payload.get("decoder")),
                    ("Direcao", payload.get("direction")),
                    ("Protocolo", payload.get("protocol")),
                    ("Valor observado", payload.get("observed_value")),
                    ("Threshold", payload.get("threshold_value")),
                    ("Unidade", payload.get("threshold_unit")),
                    ("Match", payload.get("matched")),
                    ("Flows", payload.get("flow_count")),
                    ("Bytes", payload.get("total_bytes")),
                    ("Pacotes", payload.get("total_packets")),
                    ("Motivo", payload.get("reason")),
                    ("Filtro", payload.get("clickhouse_where_summary")),
                ]
            ),
        },
        {
            "title": "Flows de amostra",
            "headers": ["flow_time", "src_ip", "src_port", "dst_ip", "dst_port", "proto_name", "bytes", "packets"],
            "rows": rows,
        },
    ]
    return simple_pdf_response("Teste do vetor", sections, "gmj-attack-vector-test.pdf")


@app.post("/api/attack-vectors/{vector_id}/test/pdf")
def test_attack_vector_pdf(
    request: Request,
    vector_id: int,
    payload: AttackVectorTestPayload | None = None,
):
    result = test_attack_vector(request, vector_id, payload)
    return attack_vector_test_pdf_response(result)
...
def security_consolidation_key(item: dict[str, Any], include_status: bool = False) -> tuple[Any, ...]:
    target_scope = ""
    if item.get("scope_type") == "internal_ip_32":
        target_scope = clean_text(item.get("target_cidr") or item.get("target_ip"))
    key: tuple[Any, ...] = (
...
def consolidated_security_anomaly_groups(status_filter: str) -> list[dict[str, Any]]:
    ...
    for group in groups.values():
        ...
        invalid_scope = bool(representative.get("invalid_scope")) or bool(scope_type == "internal_ip_32" and (not target_ip or not is_ipv4_32_cidr(target_cidr)))
        affected_count = len(src_ips or dst_ips) or len(items)
        summary = (
            f"{decoder} detectado em {target_cidr} da zona {zone_name}"
...
def anomaly_pdf_response(detail: dict[str, Any]) -> Response:
    ...
    sections = [
        {
            ...
            "rows": key_value_rows(
                [
                    ...
                    ("Total estimado", event.get("estimated_bytes")),
                    ("Alvo", event.get("target_cidr") or event.get("target_ip")),
                    ("Porta alvo", event.get("target_port")),
                    ("Origens unicas", event.get("unique_src_ips")),
                    ("Destinos unicos", event.get("unique_dst_ips")),
        ...
        {
            "title": "Top conversas",
            "headers": ["conversation", "bytes", "packets", "flow_count"],
            "rows": conversations,
        },
        {
            "title": "Flows relacionados",
            "text": f"Relatorio limitado aos primeiros {len(flows)} registros. Use CSV para extracao completa." if len(flows) >= 100 else "",
            "headers": ["flow_time", "src_ip", "src_port", "dst_ip", "dst_port", "proto_name", "bytes", "packets", "flow_count"],
            "rows": flows,
        },
    ]
...
def simple_pdf_response(title: str, sections: list[dict[str, Any]], filename: str) -> Response:
    try:
        ...
        raise HTTPException(status_code=500, detail="ReportLab nao instalado no backend") from exc
    ...
    for section in sections:
        heading = pdf_text(section.get("title"))
        if heading:
            story.append(paragraph(heading, styles["Heading2"]))
        text = pdf_text(section.get("text"))
        if text:
    ...
    sections = [
        {
            ...
            "rows": key_value_rows(
                [
                    ...
                    ("Ordenacao", f"{payload.get('order_by')} {payload.get('order_dir')}"),
                    ("Registros visiveis", len(items)),
                ]
            ),
        },
        {
...
def search_flows(
    ...
    if clean_text(format).lower() == "pdf":
        return flows_pdf_response(payload)
    return payload
